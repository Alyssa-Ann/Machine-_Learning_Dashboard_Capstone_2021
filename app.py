...
@app.route("/")
def index():
#    records = good_app_collection.find_one()
#    print(test)
#    machine_learning.model(temp)
   #cleaning.clean_data(good_app_collection)  
   return render_template("index.html", tables = [cleaning.clean_data(no_good_app_collection).to_html(classes = 'data', header = 'true')], titles = cleaning.clean_data(no_good_app_collection).columns.values)
